...
class Builder {
 ...
 /* ───────────────────────── drawGrid ───────────────────────── */
  ...
  selectElement(el, append = false) {
    ...
    if (this.bar) {
      let left = r.right + 5;
      if (left + this.bar.offsetWidth > window.innerWidth) {
        left = r.left - this.bar.offsetWidth - 5;
      }
      let top = r.top;
      if (top < 0) top = 0;

      if (top + this.bar.offsetHeight > window.innerHeight) {
        top = window.innerHeight - this.bar.offsetHeight;
        if (top < 0) top = 0;
      }
      ...
      if (this.pick) {
        ...
        this.pick.value = se.dataset.color || hex;
      }
      const right = this.anchorRight;
      const bottom = this.anchorBottom;
